...
def load_and_preprocess_predictions(
    image_path: str, enable_debug: bool, data_gen: bool, enable_cache: bool, segnet_use_gpu: bool
    ...
    debug = Debug(predictions.original, image_path, enable_debug, data_gen)
    if debug.data_gen:
        debug.db_page_index = datagen_db.add_page(image_path)
...
def detect_staffs_in_image(
    ...
    predictions, debug = load_and_preprocess_predictions(
        image_path, config.enable_debug, config.data_gen, config.enable_cache, config.segnet_use_gpu
    )
...
def main() -> None:
    ...
    parser.add_argument(
        "--datagen",
        action="store_true",
        help="Save data generated by homr. Needed for dataset building.",
    )
